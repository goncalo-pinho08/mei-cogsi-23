...
ufw allow NRPE
ufw reload
sed -i '/^allowed_hosts=/s/$/,10.5.0.5/' /usr/local/nagios/etc/nrpe.cfg
...
usermod -aG nagios
usermod -g nagios
sh -c "echo 'command[restart_tomcat]=/usr/local/nagios/libexec/restart_tomcat.sh' >> /usr/local/nagios/etc/nrpe.cfg"
